...
const CountryDetailsPage = ({country, countryBorders, returnCountryForDetails, replaceChars}) => {
  const history = useHistory();
  const {flag, name, nativeName, population, region, subregion, capital, topLevelDomain, currencies, languages} = country;

  const goBack = () => {
    history.goBack();
    returnCountryForDetails(history.location.state.from.replace('/', ''));
  }
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    history.push(`/${replaceChars(border)}`, { from: `/${name}`});
                  }}
